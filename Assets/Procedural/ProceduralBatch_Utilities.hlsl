...
void TRSm_UVs0(float2 uv,out float2 uv0,in int4 ids_)
{
    uv0=uv*_ScaleOffsets_Buffer[ids_.w].xy+_ScaleOffsets_Buffer[ids_.w].zw;
}

void TRSm_LightmapUVs(float2 uv,out float2 uv1,in int4 ids_)
{
   // float4 t=_ScaleOffset_LightMap_Buffer[dt_.lightmap_ID];
   // uv1=uv*t.xy+t.zw;
}
